...
# 定义自定义配色方案
# 将RGB颜色值转换为Matplotlib可用的十六进制格式
# R026 G008 B065 -> #1A0841
# R079 G157 B166 -> #4F9DA6
# R255 G089 B089 -> #FF5959
custom_scheme = {
    'name': 'custom_colors',
    'colors': {
        '主动推理': '#16068A',  # 深蓝色 (R022 G006 B138)
        '轮询': '#9E189D',  # 紫红色 (R158 G024 B157)
...
try:
    ...
    # 设置图表样式
    markers = {'主动推理': 'o', '轮询': 's', 'DQN': '^'}
    line_styles = {'主动推理': '-', '轮询': '--', 'DQN': '-.'}
    ...
    # 创建输出目录
    os.makedirs('fixed_boxplot_with_analysis', exist_ok=True)
    os.chdir('fixed_boxplot_with_analysis')
    ...
    print("\n所有图表绘制完成！保存在 fixed_boxplot_with_analysis 目录下")
